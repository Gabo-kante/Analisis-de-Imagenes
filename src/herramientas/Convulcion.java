 ...
 * @author gabri
 */
public class Convulcion {
    
    private Image imagenOriginal;
    private BufferedImage imagenResultante;
    private int desfase,divisor;

    public Convulcion(Image imagenOriginal) {
      this.imagenOriginal = imagenOriginal;
      // crear una nueva imagen 
      this.imagenResultante = new BufferedImage(imagenOriginal.getWidth(null), imagenOriginal.getHeight(null), BufferedImage.TYPE_INT_RGB);
    }
    
    public Image convolucionar(int [][] kernel,int desfase,int divisor){
       BufferedImage auxOriginal = herramientas.HerramientasImagen.toBufferedImage(imagenOriginal);
...
// recorrer la imagen para extraer
     ...
     return herramientas.HerramientasImagen.toImage(this.imagenResultante);
    }
    ...
    private Color generaConvolucion(int[][] ventana, int[][] kernel) {
        ...
        // validar los limites
        acumuladorRojo = verificar(acumuladorRojo);
        acumuladorVerde = verificar(acumuladorVerde);
        acumuladorAzul = verificar(acumuladorAzul);
